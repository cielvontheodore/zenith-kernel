/*
  Zenith Kernel — Linux 6.18.22, BORE scheduler, Clang/LLVM + ThinLTO,
  x86-64-v3 tuned, trimmed with a real modprobed.db from the target T480.

  Pinned to 6.18.22 specifically because that's the exact kernel version
  firelzrd's own BORE patch commit targets (bore-scheduler@0907522,
  "BORE 6.6.3 (stable) patch for 6.18.22") -- see the kernelPatchVersion
  comment below.
...
let
  ...
  kernelMajorMinor = "6.18";

  # Must stay in lockstep with the borePatch commit below -- that patch is
  # only known to apply cleanly to this exact point release (it was
  # generated by firelzrd specifically for 6.18.22, per bore-scheduler
  # commit 09075221d81b27cc6ba4dc0d1be14a62555fde85, titled
  # "BORE 6.6.3 (stable) patch for 6.18.22"). Don't bump this independently
  # of the patch -- see patches/README.md before changing either.
  kernelPatchVersion = "22";
  ...
  borePatch = fetchpatch {
    # Pinned to the immutable commit that introduced this exact patch
    # ("BORE 6.6.3 (stable) patch for 6.18.22"), not the mutable `main`
    # branch -- so this can never silently change out from under the
    # build. Hash verified locally via `nix store prefetch-file` against
    # this exact URL (not re-derived or guessed).
    url = "https://raw.githubusercontent.com/firelzrd/bore-scheduler/09075221d81b27cc6ba4dc0d1be14a62555fde85/patches/stable/linux-6.18-bore/0001-linux6.18.22-bore-6.6.3.patch";
    hash = "sha256-mLJzD7p19NU7pPsY2R16sJQV/qsOgozVtki0RxzOvk4=";
  };
